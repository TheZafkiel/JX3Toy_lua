...
ZuoZhe = 'YUI'
--奇穴
QiXue = '[扬戈][神勇][骁勇][击水][劲风][风虎][牧云][战心][破楼兰][激雷][龙血][虎贲]'
--其他说明
ShuoMing = ' 自动补扶摇 自己跳 锈铁钩锁|景流.天地一|劈山尾鞭 读条50%会 ,输出会暂停3秒 \n 破楼兰自动雷 新手适用 注意开自动雷会导致你雷的收益无法最大化 熟悉之后清删掉自动雷手动打雷'
...
function DPS()
    ...
    if  nobuff('激雷') and rage() >4 then
        cast('撼如雷',true)
    end
    if rage() < 4 and tbufftime('流血') < 2.5 then
        cast('龙吟')
    end
    if rage() >=4 then
        cast('龙牙')
