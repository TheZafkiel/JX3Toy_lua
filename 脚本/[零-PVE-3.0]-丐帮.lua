...
ZuoZhe = 'YUI'
--奇穴
QiXue = '玄黄,御龙,自强,无疆,益元,越渊,满盈,驯致,贞固,复礼,饮江,潜龙勿用'
--其他说明
ShuoMing = ' 锈铁钩锁|景流.天地一|劈山尾鞭 读条50%会自动跳,输出会暂停3秒 -丐帮跳nima 自己跳  '
...
function DPS()
    ...
    -------------------------分割线------------------------------------
    if nobuff('5994') then ---6377 无疆
        fcast('酒中仙')
    end
    if nobuff('伏虎') or bufftime('伏虎') < 3 then
        cast('龙战于野')
    end
    cast('亢龙有悔')
    cast('潜龙勿用')
