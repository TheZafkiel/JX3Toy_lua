...
ZuoZhe = 'YUI'
--奇穴
QiXue = '星旗,秋霁,破月,风骨,重霄,渊岳,玄肃,王师,百节,忘断,徵逐,青山共我'
--其他说明
ShuoMing = '自动补扶摇 自己跳 \n  贴着BOSS起手 傲龙岛 请注意不需要贴身的时候关闭宏'
...
function DPS()
    ...
    if (nobuff('忘断') and dis() >= 6) or (cn('寂洪荒') == 0 and cd('寂洪荒')) then
        cast('血覆黄泉')
        cast('金戈回澜')
    end
    if dis() >= 6 and (cn('寂洪荒') == 0 and cd('寂洪荒')) then
        cast('孤风飒踏')
    end

    if dis() < 6 and tnobuff('链接') then
        cast('日月吴钩')
    end
